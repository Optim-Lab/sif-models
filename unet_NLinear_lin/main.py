...
def main():
    ...
    if key is not None:
        ...
        wandb.login(key=key)
        wandb.init(project='Unet_NLinear_lin', name=name)
    ...
    with tqdm(range(1,  epochs+1)) as tr:
        for epoch in tr:
            ...
            if valid_loss < best_valid_loss:
                ...
                torch.save(model.state_dict(), 'best_unet_Nlinear_lin.pth')
    ...
    model = UnetNLinear(input_window, output_window, de).to(device)
    model.load_state_dict(torch.load('best_unet_Nlinear_lin.pth'))
