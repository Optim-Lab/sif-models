...
def get_score(pred, target, output_window, device):
    ...
    for k in range(pred.shape[0]):
        save_image(target[k], './unet_dlinear_lin_plot/target/target_{}.png'.format(k))
        save_image(pred[k], './unet_dlinear_lin_plot/pred/pred_{}.png'.format(k))
    ...
    for k in range(output_window):
        ...
        pred_path  = './unet_dlinear_lin_plot/pred/pred_{}.png'.format(k)
        target_path = './unet_dlinear_lin_plot/target/target_{}.png'.format(k)
