...
def main():
    ...
    if key is not None:
        ...
        wandb.login(key=key)
        wandb.init(project='Unet_NLinear_upconv', name=name)
    ...
    with tqdm(range(1,  epochs+1)) as tr:
        for epoch in tr:
            ...
            if valid_loss < best_valid_loss:
                ...
                torch.save(model.state_dict(), 'best_unet_Nlinear_upconv.pth')
    ...
    model = UnetNLinear(input_window, output_window).to(device)
    model.load_state_dict(torch.load('best_unet_Nlinear_upconv.pth'))
