...
#%%    
class SINDUnet(nn.Module):
    def __init__(self, input_window, output_window, de):
        super(SINDUnet,self).__init__()
        self.decomp = series_decomp(de) # moving average
        self.unet = Unet(input_window, output_window) # real Unet
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        seq_last_m = moving_mean[:,-1:,:,:].detach()
        moving_mean = moving_mean - seq_last_m
        moving_mean = self.unet(moving_mean)
        moving_mean = moving_mean + seq_last_m
        moving_mean = self.upconv(moving_mean)
        
        seq_last_r = res[:,-1:,:,:].detach()
        #res = self.upconv(res)
        res = res - seq_last_r
        res = self.upconv(res)
        #res = self.conv(res)
