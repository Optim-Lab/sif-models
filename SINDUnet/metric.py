...
def get_score(pred, target, output_window, device):
    ...
    for k in range(pred.shape[0]):
        save_image(target[k], './sindunet_plot/target/target_{}.png'.format(k))
        save_image(pred[k], './sindunet_plot/pred/pred_{}.png'.format(k))
    ...
    for k in range(output_window):
        ...
        pred_path  = './sindunet_plot/pred/pred_{}.png'.format(k)
        target_path = './sindunet_plot/target/target_{}.png'.format(k)
