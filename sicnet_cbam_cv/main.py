...
def main():
    ...
    if key is not None:
        ...
        wandb.login(key=key)
        wandb.init(project='sicnet_cbam_cv', name=name)
    ...
    with tqdm(range(1,  epochs+1)) as tr:
        for epoch in tr:
            ...
            if valid_loss < best_valid_loss:
                ...
                torch.save(model.state_dict(), './saved_models/best_sicnet_cbam_cv_{}_MSE_{}.pth'.format(seed, ci))
    ...
    model = SICNet(input_window, output_window).to(device)
    model.load_state_dict(torch.load('./saved_models/best_sicnet_cbam_cv_{}_MSE_{}.pth'.format(seed, ci)))
