...
def main():
    ...
    if key is not None:
        ...
        wandb.login(key=key)
        wandb.init(project='Unet_DLinear_upconv', name=name)
    ...
    with tqdm(range(1,  epochs+1)) as tr:
        for epoch in tr:
            ...
            if valid_loss < best_valid_loss:
                ...
                torch.save(model.state_dict(), 'best_unet_Dlinear_upconv.pth')
    ...
    model = UnetDLinear(input_window, output_window, de).to(device)
    model.load_state_dict(torch.load('best_unet_Dlinear_upconv.pth'))
