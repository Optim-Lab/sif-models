...
def main():
    ...
    if key is not None:
        ...
        wandb.login(key=key)
        wandb.init(project='UnetNODE_dopri5', name=name)
    ...
    with tqdm(range(1,  epochs+1)) as tr:
        for epoch in tr:
            ...
            if valid_loss < best_valid_loss:
                ...
                torch.save(model.state_dict(), 'best_unet_NODE_dopri5.pth')
    ...
    model = UnetNODE(input_window, output_window, de, pe).to(device)
    model.load_state_dict(torch.load('best_unet_NODE_dopri5.pth'))
