...
def main():
    ...
    if key is not None:
        ...
        wandb.login(key=key)
        wandb.init(project='ablation_study_data_cv', name=name)
    ...
    model = UnetNODE(input_window, output_window, de).to(device)
    ...
    with tqdm(range(1,  epochs+1)) as tr:
        for epoch in tr:
            ...
            if valid_loss < best_valid_loss:
                ...
                torch.save(model.state_dict(), './saved_models/best_ablation_study_data_cv_{}_k{}_{}.pth'.format(seed, de, ci))
    ...
    model = UnetNODE(input_window, output_window, de).to(device)
    model.load_state_dict(torch.load('./saved_models/best_ablation_study_data_cv_{}_k{}_{}.pth'.format(seed, de, ci)))
